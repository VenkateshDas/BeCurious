# I was wrong about why exercise makes you feel better. So was everyone else.
...
```mermaid
flowchart LR
    A["The Puzzle<br/>Why does it work?"] --> B["The Wrong Answer<br/>Endorphins were blamed<br/>for decades"]
    B --> C["The Real Chemistry<br/>Your body's own systems<br/>fire within minutes"]
    C --> D["The Brain<br/>What physically changes<br/>inside the skull"]
    D --> E["The Long Game<br/>What weeks and months<br/>of movement builds"]
    E --> F["Your Toolkit<br/>What to use,<br/>when, and why"]

    style A fill:#023e8a,color:#fff
    style B fill:#e63946,color:#fff
    style C fill:#e76f51,color:#fff
    style D fill:#9b5de5,color:#fff
...
```mermaid
flowchart TD
    THEORY["The theory that lasted decades<br/>Exercise → Endorphins → Good mood"]

    TEST["The experiment<br/>Give naltrexone to block all endorphin receptors<br/>Then ask participants to exercise"]

    EXPECTED["If the theory was right<br/>No endorphins acting → No euphoria"]
    ACTUAL["What actually happened<br/>Euphoria came anyway<br/>Completely unaffected"]

    QUESTION["If not endorphins — what?"]
    ...
    style THEORY fill:#6c757d,color:#fff
    style TEST fill:#023e8a,color:#fff
    style EXPECTED fill:#adb5bd,color:#000
    style ACTUAL fill:#e63946,color:#fff
    style QUESTION fill:#2d6a4f,color:#fff
```
...
Here's the part that actually matters. Endorphins are large molecules. They cannot cross the **blood-brain barrier** — the protective filter that controls what enters the brain from the bloodstream. They flood the body but never reach the brain directly.
...
```mermaid
flowchart LR
    subgraph BLOOD["In the bloodstream"]
        END["Endorphins<br/>(large molecules)"]
        ANA["Anandamide<br/>(small, fat-soluble)"]
    end

    subgraph BBB["Blood-Brain Barrier<br/>(protective filter)"]
        GATE["Controls what enters the brain"]
    end

    subgraph BRAIN["Inside the brain"]
        BLOCKED["Cannot act here<br/>No direct mood effect"]
        WORKS["Calm euphoria<br/>Anxiety drops<br/>Problems feel lighter"]
    end

    END -->|"Too large — blocked"| GATE
    GATE --> BLOCKED
    ANA -->|"Small enough — passes freely"| GATE
    GATE --> WORKS

    style END fill:#e63946,color:#fff
    style ANA fill:#52b788,color:#fff
    style GATE fill:#023e8a,color:#fff
    style BLOCKED fill:#adb5bd,color:#000
    style WORKS fill:#1b4332,color:#fff
```
...
```mermaid
flowchart LR
    MOV(["Movement"]) --> D & S & N & A

    D["Dopamine<br/>Reward Signal"]
    S["Serotonin<br/>Mood Anchor"]
    N["Norepinephrine<br/>Clarity Signal"]
    A["Anandamide<br/>Calm Euphoria"]

    D --> DE["Motivation rises<br/>Sense of accomplishment<br/>Forward-leaning feeling"]
    S --> SE["Mood stabilises<br/>Less irritability<br/>Steadier baseline"]
    N --> NE["Thinking clears<br/>Brain sharpens<br/>Signal-to-noise improves"]
    A --> AE["Anxiety drops<br/>Problems feel lighter<br/>Calm without sedation"]

    style MOV fill:#2d6a4f,color:#fff
    style D fill:#e76f51,color:#fff
    style S fill:#9b5de5,color:#fff
    style N fill:#0096c7,color:#fff
...
There's a command centre in the body called the **HPA Axis** — Hypothalamic-Pituitary-Adrenal axis. Three glands (two in the brain, one above each kidney) that work together to manage the stress response. When you're stressed, this system releases **cortisol**, the primary stress hormone.
...
The second mechanism is the **autonomic nervous system** — the part of your nervous system that runs automatically, controlling heart rate, digestion, and breathing. It has two modes:

- **Sympathetic** (fight-or-flight): alert, activated. The accelerator pedal.
- **Parasympathetic** (rest-and-digest): calm, recovery. The brake pedal.
...
```mermaid
flowchart LR
    STR["Stretching<br/>+ Slow breathing"]
    VN["Vagus nerve<br/>activates"]
    PS["Parasympathetic<br/>tone rises"]
    GABA["GABA increases<br/>(brain's calm signal)"]
    RESULT["Neural noise quiets<br/>Anxiety drops<br/>Mind can switch off"]

    STR --> VN --> PS --> GABA --> RESULT

    style STR fill:#9b5de5,color:#fff
    style VN fill:#7b2d8b,color:#fff
    style PS fill:#560bad,color:#fff
    style GABA fill:#3a0ca3,color:#fff
...
```mermaid
flowchart TD
    IDLE["Idle or stressed state"]
    DMN["Default Mode Network activates"]
    LOOP["Self-referential thought loop<br/>Replaying memories<br/>Imagining worst-case futures"]
    WORSE["Mood worsens"]

    IDLE --> DMN --> LOOP --> WORSE --> IDLE

    START(["You start moving"])
    OUT["Attention shifts outward<br/>Body in motion<br/>Environment awareness"]
    QUIET["DMN deactivates<br/>Rumination loses fuel"]
    BETTER(["Mood lifts"])

    START --> OUT --> QUIET --> BETTER

    LOOP -. "movement interrupts here" .-> OUT

    style IDLE fill:#c1121f,color:#fff
    style DMN fill:#d62828,color:#fff
    style LOOP fill:#e63946,color:#fff
    style WORSE fill:#c1121f,color:#fff
    style START fill:#2d6a4f,color:#fff
    style OUT fill:#40916c,color:#fff
    style QUIET fill:#52b788,color:#fff
    style BETTER fill:#1b4332,color:#fff
```
...
```mermaid
flowchart TD
    subgraph DECLINE["Without exercise — The Decline"]
        D1["Chronic stress and depression"]
        D2["BDNF depletes"]
        D3["Hippocampus shrinks<br/>(visible in brain scans)"]
        D4["Memory and mood worsen"]
        D1 --> D2 --> D3 --> D4 --> D1
    end

    subgraph REPAIR["With regular exercise — The Repair"]
        R1["Aerobic movement<br/>20-40 min, most days"]
        R2["BDNF rises"]
        R3["Neurogenesis begins<br/>New neurons born in hippocampus"]
        R4["Hippocampus grows back"]
        R5["Mood and memory improve"]
        R1 --> R2 --> R3 --> R4 --> R5
    end

    D1 -. "start exercising" .-> R1

    style D1 fill:#e63946,color:#fff
    style D2 fill:#d62828,color:#fff
    style D3 fill:#c1121f,color:#fff
    style D4 fill:#9d0208,color:#fff
    style R1 fill:#40916c,color:#fff
    style R2 fill:#52b788,color:#fff
    style R3 fill:#74c69d,color:#000
    style R4 fill:#95d5b2,color:#000
...
```mermaid
timeline
    title Timeline of Effects After You Start Moving
    0 to 2 min  : Body state shifts
                : Blood flow to brain increases
                : Attention turns outward
                : Thought loop breaks
    2 to 10 min : Monoamines fire
                : Dopamine, Serotonin, Norepinephrine rise
                : Endocannabinoids release
                : DMN begins quieting
    10 to 30 min : HPA Axis recalibrates
                 : Cortisol normalises
                 : HRV improves
    Days to Weeks : BDNF accumulates
                  : Inflammation reduces
                  : Self-efficacy builds
    Months : Neurogenesis in hippocampus
           : Structural brain repair complete
           : Emotional regulation deepens
```
...
```mermaid
flowchart TD
    Q{"What state<br/>are you in?"}

    Q -->|"Wired, anxious,<br/>cannot switch off"| STR["Stretching or Yoga"]
    Q -->|"Flat, low energy,<br/>stuck in a spiral"| WALK["Brisk Walking"]
    Q -->|"Want long-term<br/>brain change"| AER["Vigorous Aerobic"]

    STR --> SR["GABA rises<br/>Vagus nerve activates<br/>Cortisol drops<br/>Mind quiets"]
    WALK --> WR["Monoamines fire<br/>Endocannabinoids release<br/>DMN suppresses<br/>Energy returns"]
    AER --> AR["Large endocannabinoid spike<br/>Strong BDNF release<br/>Neuroplasticity over weeks"]

    SR --> OUT1(["Outcome: Calm"])
    WR --> OUT2(["Outcome: Lifted mood"])
    AER --> OUT3(["Outcome: Structural repair"])

    style Q fill:#023e8a,color:#fff
    style STR fill:#9b5de5,color:#fff
    style WALK fill:#e76f51,color:#fff
    style AER fill:#e63946,color:#fff
    style OUT1 fill:#9b5de5,color:#fff
    style OUT2 fill:#e76f51,color:#fff
    style OUT3 fill:#e63946,color:#fff
```
...
```mermaid
flowchart TD
    START(["A single step. You move."])

    START --> S1["Step 1 — Loop breaks<br/>Inward attention shifts outward"]
    S1 --> S2["Step 2 — Monoamines fire<br/>Dopamine, Serotonin, Norepinephrine"]
    S2 --> S3["Step 3 — Endocannabinoids release<br/>Anandamide crosses blood-brain barrier"]
    S3 --> S4["Step 4 — Stress system recalibrates<br/>Cortisol normalises, HRV improves"]
    S4 --> S5["Step 5 — GABA rises<br/>Neural noise quiets, Anxiety drops"]
    S5 --> S6["Step 6 — Default Mode Network quiets<br/>Rumination engine loses power"]
    S6 --> S7["Step 7 — BDNF builds over time<br/>Neuroplasticity increases, Brain grows denser"]
    S7 --> S8["Step 8 — Inflammation reduces<br/>Brain chemistry clears"]
    S8 --> S9["Step 9 — Psychology reinforces<br/>Self-efficacy and mastery accumulate"]
    S9 --> S10["Step 10 — Context amplifies<br/>Green space, Novelty, Outdoors"]

    S10 --> END(["You feel better. Every time."])

    style START fill:#023e8a,color:#fff
    style S1 fill:#e9c46a,color:#000
    style S2 fill:#f4a261,color:#000
    style S3 fill:#e76f51,color:#fff
    style S4 fill:#e63946,color:#fff
    style S5 fill:#9b5de5,color:#fff
    style S6 fill:#480ca8,color:#fff
    style S7 fill:#0096c7,color:#fff
    style S8 fill:#0077b6,color:#fff
    style S9 fill:#40916c,color:#fff
  ...
  root((Movement))
    ...
    Minutes later
      Monoamines fire
        ...
        Norepinephrine
      Endocannabinoids
        Anandamide crosses barrier
        Calm and reduced anxiety
    Stress system
    ...
    Over weeks and months
      ...
      Inflammation reduces
      Self-efficacy grows
    Environment multiplies
      Green space boosts effect
      Novel environments
        Suppress DMN further
```
